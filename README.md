...
In Completed orders & receipts, use Flag completed order and enter a required reason. The order remains completed and its flag, actor, timestamp, and explanation are saved with its history. Flagged receipts display the reason when printed. Admins and super admins can use Edit flag note to update the explanation. Notes remain required and limited to 500 characters. Each edit records the previous note, new note, editor and time in the order history. The original flag timestamp and author remain unchanged; stale edits are rejected so another admin's changes are not silently overwritten. Conditional writes prevent concurrent changes from silently replacing the audit history. Deploy the backend and sync Hosting outputs before publishing this frontend. Run `node --test tests/admin-order-actions.test.mjs tests/receipts.test.mjs` for authorization, lifecycle, note validation, and receipt tests.

### Users and roles

Only super admins can open Account > Users & roles. The directory queries Cognito in pages of 20, supports email-prefix searches, and provides confirmation before role changes. Admins and regular users cannot list users or change any roles, including through direct API requests. Super admins can assign User, Admin, or Super Admin membership. Self-role changes are blocked. Other custom Cognito groups are preserved. The backend checks the caller's current groups on every request and logs successful role changes. New role claims become effective when the affected user signs in again or refreshes their session. Role changes are not a replacement for revoking an existing session.
...
Flagged receipts show Flagged by and Last edited by, with an expandable flag history containing each action, actor name, role, Philippine timestamp, and note revision. New flag actions snapshot the actor's first/last name from Cognito on the server; client-supplied names are not trusted. Older records resolve first and last names through a receipt-scoped, admin-only Cognito lookup. Unavailable names display "Name unavailable"; account IDs stay internal and never appear in the flag UI or PDF. The original flagger and timestamp are retained when notes change. Printed/PDF receipts include this history.

Admins and super admins can expand **View order activity** on order cards and completed receipts to see each recorded step, the person responsible, their role, timestamp, and notes. Printed receipts include the full activity history. New placement and status changes save the actor name on the server; older steps resolve recorded actor IDs within that order only. Missing historical actors show "Name unavailable".
