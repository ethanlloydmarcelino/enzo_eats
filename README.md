...
Flagged receipts show Flagged by and Last edited by, with an expandable flag history containing each action, account ID, role, Philippine timestamp, and note revision. New flag actions snapshot the actor's first/last name from Cognito on the server; client-supplied names are not trusted. Older records without a name display their saved account ID. The original flagger and timestamp are retained when notes change. Printed/PDF receipts include this history.
