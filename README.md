...
The hosting build copies `config/sandbox-outputs.json` to `amplify_outputs.json` before exporting the web app. This checked-in client configuration points to the existing `ethan` sandbox in `ap-southeast-1` and contains public resource identifiers and API metadata, not AWS credentials. Hosting does not need CloudFormation access to the sandbox account. This configuration shares the development sandbox; it does not deploy a separate branch backend. After recreating or changing the backend, refresh the snapshot from the locally generated `amplify_outputs.json`, check that it contains no secrets, and commit it. Commit and push `amplify.yml` to use the repository's build settings instead of the console-only settings.
