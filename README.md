...
The hosting build generates `amplify_outputs.json` from the existing `ethan` sandbox in `ap-southeast-1` before exporting the web app. The Amplify Hosting service role must have permission to read that backend's outputs. This configuration shares the development sandbox; it does not deploy a separate branch backend. Commit and push `amplify.yml` to use the repository's build settings instead of the console-only settings.
